...
fn selector_by_config(s: &config::DeviceSelector) -> Box<dyn udevdetect::DevFilter + Send + Sync> {
    match s {
        ...
        config::DeviceSelector::USBIDClass {
            vendor,
            model,
            class,
...
fn main() {
    ...
    let conf = config::read_config(config_path).expect("Failed to read config file");
        ...
        match dev {
            config::Device::Simple {
                ...
                let t = thread::spawn(move || {
                    ...
                    let dev_ev_listener = dl.listen().expect("Failed to listen to udev events");
